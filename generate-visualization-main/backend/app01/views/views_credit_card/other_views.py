...
def RecreateTable(request):
    ...
    print("重建表开始")
    api_DataGeneration.api_user_table("信用卡违规套现")
    api_DataGeneration.api_store_table("信用卡违规套现")
    api_DataGeneration.api_card_table("信用卡违规套现")
    api_DataGeneration.api_trans_table("信用卡违规套现")
    print("重建表结束")
...
def do_zip_compress(dirpath):
    ...
    output_name = f"{dirpath}信用卡违规套现.zip"
    print(output_name)
...
def download(request):
    ...
    print(path)
    api_DataGeneration.api_export_to_csv("信用卡违规套现", path)
