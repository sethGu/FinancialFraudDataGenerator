...
def RecreateTable(request):
    ...
    api_DataGeneration.api_user_table("Credit_card_fraud")
    api_DataGeneration.api_store_table("Credit_card_fraud")
    api_DataGeneration.api_card_table("Credit_card_fraud")
    api_DataGeneration.api_trans_table("Credit_card_fraud")
...
def do_zip_compress(dirpath):
    ...
    output_name = f"{dirpath}Credit_card_fraud.zip"
    print(output_name)
...
def download(request):
    ...
    print(path)
    api_DataGeneration.api_export_to_csv("Credit_card_fraud", path)

    print("Start packing data")
    do_zip_compress(path)

    print('Start downloading data')
    file_path = path + r'Credit_card_fraud.zip'
    try:
        r = StreamingHttpResponse(open(file_path, "rb"))
        r["content_type"] = "application/octet-stream"
        r["Content-Disposition"] = "attachment;filename=CreditCardCashOut.zip"
        return r
    except Exception:
