...
from app01.models import StoreUser, StoreStore, StoreCard
...
def user(request):
    ...
    api_DataGeneration.api_user("Merchant_violation", total)
...
def userDisplay():
    ...
    user_number = StoreUser.objects.count()
...
def store(request):
    ...
    api_DataGeneration.api_store("Merchant_violation", total)
...
def storeDisplay():
    stores = StoreStore.objects.all()
    stores_number = StoreStore.objects.count()
...
def card(request):
    ...
    api_DataGeneration.api_card("Merchant_violation")
