...
from app01.models import GamblingTrans
...
def consumption(request):
    ...
    api_DataGeneration.api_consumption("Gambling_violation", year, month, day, duration)
...
def consumptionDisplay():
    consumption_num = GamblingTrans.objects.filter(abnormal=0, t2='01').count()
    consumption_lists = GamblingTrans.objects.filter(abnormal=0, t2='01').values('t17', 't2', 't23', 't19').order_by('t23', 't19')
...
def transfer(request):
    ...
    api_DataGeneration.api_relative("Gambling_violation", year, month, day, duration)
...
def transferDisplay():
    transfer_num = GamblingTrans.objects.filter(abnormal=0, t2='03').count()
    transfer_lists = GamblingTrans.objects.filter(abnormal=0, t2='03').values('t17', 't2', 't23', 't19').order_by('t23', 't19')
    # print(transfer_num)
