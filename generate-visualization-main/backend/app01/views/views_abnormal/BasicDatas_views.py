...
from app01.models import AbnormalUser, AbnormalStore, AbnormalCard
...
def user(request):
    ...
    api_DataGeneration.api_user("Abnormal_transfer", total)
...
def userDisplay():
    users = AbnormalUser.objects.all()
    user_number = AbnormalUser.objects.count()
...
def store(request):
    ...
    api_DataGeneration.api_store("Abnormal_transfer", total)
...
def storeDisplay():
    stores = AbnormalStore.objects.all()
    stores_number = AbnormalStore.objects.count()
...
def card(request):
    ...
    api_DataGeneration.api_card("Abnormal_transfer")
...
def cardDisplay():
    cards = AbnormalCard.objects.all()
