...
def consumption(request):
    ...
    duration = p['duration']
    print("生成正常消费开始")
    api_DataGeneration.api_consumption("异常转账", year, month, day, duration)
    print("生成正常消费结束")
...
def consumptionDisplay():
    consumption_num = AbnormalTrans.objects.filter(abnormal=0, t2='01').count()  # 查询正常消费数据的数量
...
def transfer(request):
    ...
    print("生成正常转账开始")
    api_DataGeneration.api_relative("异常转账", year, month, day, duration)
    print("生成正常转账结束")
...
def transferDisplay():
    transfer_num = AbnormalTrans.objects.filter(abnormal=0, t2='03').count()  # 查询正常转账数据的数量
