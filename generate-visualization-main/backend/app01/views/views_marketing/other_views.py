...
def RecreateTable(request):
    ...
    api_DataGeneration.api_user_table("Scalper_marketing")
    api_DataGeneration.api_store_table("Scalper_marketing")
    api_DataGeneration.api_card_table("Scalper_marketing")
    api_DataGeneration.api_trans_table("Scalper_marketing")
...
def do_zip_compress(dirpath):
    ...
    output_name = f"{dirpath}Scalper_marketing.zip"
    print(output_name)
...
def download(request):
    ...
    print(path)
    api_DataGeneration.api_export_to_csv("Scalper_marketing", path)

    print("Start packing data")
    do_zip_compress(path)

    print('Start downloading data')
    file_path = path + r'Scalper_marketing.zip'
    try:
        r = StreamingHttpResponse(open(file_path, "rb"))
        r["content_type"] = "application/octet-stream"
        r["Content-Disposition"] = "attachment;filename=MarketingFraud.zip"
        return r
    except Exception:
