...
from app01.models import RegisterTrans
...
def consumption(request):
    ...
    duration = p['duration']
    print("生成正常消费开始")
    api_DataGeneration.api_consumption("伪冒注册欺诈", year, month, day, duration)
    print("生成正常消费结束")
...
def consumptionDisplay():
    consumption_num = RegisterTrans.objects.filter(abnormal=0, t2='01').count()  # 查询正常消费数据的数量
    consumption_lists = RegisterTrans.objects.filter(abnormal=0, t2='01').values('t17', 't2', 't23', 't19').order_by('t23', 't19')
...
def transfer(request):
    ...
    print("生成正常转账开始")
    api_DataGeneration.api_relative("伪冒注册欺诈", year, month, day, duration)
    print("生成正常转账结束")
...
def transferDisplay():
    transfer_num = RegisterTrans.objects.filter(abnormal=0, t2='03').count()  # 查询正常转账数据的数量
    transfer_lists = RegisterTrans.objects.filter(abnormal=0, t2='03').values('t17', 't2', 't23', 't19').order_by('t23', 't19')
    # print(transfer_num)
