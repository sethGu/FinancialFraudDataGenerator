...
export default {
  ...
  beforeCreate() {
    this.$store.dispatch('register/durationSetRegister').then(() => {
      this.form.startDate = this.$store.state.register.register_date_data.startDate
      this.duration = this.$store.state.register.register_date_data.duration
    })
  },
  created() {
    this.init()
  ...
  methods: {
    conC(value) {
      this.is_con = value
      this.is_tran = value
      // console.log(this.is_con)   true or false
    },
    tranC(value) {
      this.is_tran = value
    },
    dispatchCon() {
      if (this.is_con) {
        ...
        return this.$store.dispatch('register/consumeGenerateRegister', { date: this.form.startDate, duration: this.duration })
      } else {
        console.log('consumeInit')
        return this.$store.dispatch('register/consumeInitRegister', { date: this.form.startDate, duration: this.duration })
      }
    },
    dispatchTran() {
      if (this.is_tran) {
        console.log('transferGenerate')
        return this.$store.dispatch('register/transferGenerateRegister', { date: this.form.startDate, duration: this.duration })
      } else {
        console.log('transferInit')
        return this.$store.dispatch('register/transferInitRegister', { date: this.form.startDate, duration: this.duration })
      }
    },
    reset() {
      this.userAmount = 50
    ...
    init() {
      this.$store.dispatch('register/userInitRegister', this.userAmount).then(() => {
        ...
        this.$store.dispatch('register/storeInitRegister', this.storeAmount).then(() => {
          ...
          this.$store.dispatch('register/cardInitRegister', '1').then(() => {
            ...
            this.$store.dispatch('register/consumeInitRegister').then(() => {
              ...
              // this.$store.dispatch('register/transferInitRegister').then(() => {
              // this.showChartTrans()

              this.$store.dispatch('register/registerFraudInit').then(response => {
                this.showChartRegisterFraud()
                this.showChartFraud()
              })
              // })
            })
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.$confirm('Do you want to generate the data?', 'Tip', {
            ...
            //   fullscreen: false
            // })
            this.$store.dispatch('register/durationChooseRegister',
              { startDate: this.form.startDate, duration: this.duration }).then(() => {
              console.log(this.form.startDate)
              this.$store.dispatch('register/userGenerateRegister', this.userAmount).then(() => {
                loadingInstanceUserAge.close()
                // loadingInstanceUserJob.close()
                this.showUserAge()
                // this.showUserJob()

                this.$store.dispatch('register/storeGenerateRegister', this.storeAmount).then(() => {
                  loadingInstanceStoreBig.close()
                  this.showChartStoreBig()

                  this.$store.dispatch('register/cardGenerateRegister', '1').then(() => {
        ...
        xAxis: [
          {
            type: 'category',
            name: 'Age',
            data: this.register_user_data['user_age_data_1'],
        ...
        series: [
          {
            ...
            data: this.register_user_data['user_age_data_2']
          }
        ]
      }
    ...
    showChartCard() {
      ...
      this.register_card_data.forEach(item => {
        var objTmp = {
          value: item.value,
          name: item.name
        }
        ...
        series: [
          {
            ...
            data: this.registerFraud_data['data_1']
          }
        ]
      }
      this.chart.setOption(optionRegisterFraud)
    },
    showChartConsume() {
      ...
      var optionConsume = {
        ...
        xAxis: {
          ...
          data: this.register_consume_data['trans_time']
        },
        yAxis: {
          type: 'value',
          name: 'Amount (CNY)'
        ...
        series: [
          {
            ...
            data: this.register_consume_data['trans_amount']
          }
        ]
      }
    ...
    showChartFraud() {
      ...
      var optionFraud = {
        ...
        xAxis: {
          ...
          // data: ['20151212', '20161212', '20181212', '20191212', '20201212', '20211212', '20221212']
          data: this.registerFraud_data['data_2']['trans_time']
        },
        yAxis: {
          type: 'value',
          name: 'Amount (CNY)'
        ...
        series: [
          {
            ...
            stack: 'Total',
            // data: [120, 132, 101, 134, 90, 230, 210]
            data: this.registerFraud_data['data_2']['trans_amount']
          }
        ]
      }
    ...
    //   },
    // data: ['20151212', '20161212', '20181212', '20191212', '20201212', '20211212', '20221212']
    //   data: this.register_transfer_data['trans_time']
    // },
    // yAxis: {
    //   type: 'value',
    //   name: 'Amount (CNY)'
    ...
    //     stack: 'Total',
    // data: [120, 132, 101, 134, 90, 230, 210]
    //       data: this.register_transfer_data['trans_amount']
    //     }
    //   ]
    // }
    // }
